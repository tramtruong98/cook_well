        ...
        <div class="container">
            <div class="row no-gutters slider-text align-items-center justify-content-center">
                <div class="col-md-9 ftco-animate text-center">
                    <p class="breadcrumbs"><span class="mr-2"><a href="index.html">My</a></span> <span>Blog</span></p>
                    <h1 class="mb-0 bread">Blog</h1>
                    <p><a href="#" type="submit" class="btn btn-primary" data-toggle="modal" data-backdrop="static"
                            data-keyboard="false" data-target="#myModal">Create my recipe!</a></p>
                </div>
            </div>
        </div>
    </div>
                            ...
                            <div class="col-md-12 d-flex ftco-animate">
                                <div class="blog-entry align-self-stretch d-md-flex">
                                    ...
                                    <div class="text d-block pl-md-4">
                                        <div class="meta mb-3">
                                            <div>{{ $recipe->created_at->format('d M Y') }}</a></div>
                                            <div>It's me</a></div>
                                            @php
                                                $item = $recipe->post
                                            @endphp
                                            <div><span class="flaticon-heart-shape-outline"></span> {{ $item->likers()->count() }}</a></div>
                                        </div>
                                        <h3 class="heading"><a href="/blog/{{ $item->id }}">{{ $recipe->course->name }}</a></h3>
                                        <p>{{ $recipe->title }}</p>
                                        <p>
                                            <div class="inline-block">
                                                <form action="/post/delete/{{ $item->id }}" method="POST">
                                                    @csrf
                                                    {{-- @method('DELETE') --}}
                                                    <button type="submit" class="btn btn-primary py-2 px-3">Delete</button>
                                                </form>
                                            </div>
                                        </p>
                                        {{-- <p><a href="/blog/{{ $recipe->post->id }}" class="btn btn-primary py-2 px-3">Delete this recipe</a></p> --}}
                                    </div>
                                </div>
                            </div>
                        @endforeach
